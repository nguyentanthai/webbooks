...
function Validator(options) {
    ...
    if(formElement) {
        formElement.onsubmit = function(e) {
            ...
            }else { //Trường hợp submit vs hành vi mặc định
                //formElement.submit()
            }
        }
...
//2. Khi hợp lệ => Không trả ra cái gì cả (undefine)
...
Validator.isConfirmed = function(selector, getConfirmValue, message) {
    return {
        selector: selector,
        test: function(value) {
            return value === getConfirmValue() ? undefined  : message || 'Mật khẩu nhập vào không chính xác' 
        }
    }
}